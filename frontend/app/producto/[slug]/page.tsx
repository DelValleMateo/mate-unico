                            ...
                            <div key={r.id} className="bg-[#141414] p-6 rounded-2xl border border-gray-800 shadow-xl transition hover:border-gray-600">
                                <div className="flex items-center justify-between mb-4">
                                    <span className="text-[10px] font-bold uppercase tracking-widest text-gray-500">
                                        {r.attributes?.users_permissions_user?.data?.attributes?.username || r.users_permissions_user?.username || "Comprador Anónimo"}
                                    </span>
                                    <div className="flex gap-0.5">
                                        {[...Array(5)].map((_, i) => (
                                            <Star 
                                                key={i} 
                                                className={`w-3 h-3 ${i < (r.attributes?.estrellas || r.estrellas || 0) ? "fill-yellow-500 text-yellow-500" : "text-gray-800"}`} 
                                            />
                                        ))}
                                    </div>
                                </div>
                                <p className="text-gray-300 italic text-sm leading-relaxed">
                                    "{r.attributes?.comentario || r.comentario}"
                                </p>
                            </div>
                        ))
                    ) : (
